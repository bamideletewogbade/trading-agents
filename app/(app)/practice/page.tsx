          ...
          return (
            <li key={lab.key}>
              {lab.href ? (
                <Link
                  href={lab.href}
                  className="flex items-center gap-3 rounded-md border border-line bg-panel p-4 hover:border-edge"
                >
                  <div className="min-w-0 flex-1">
                    <h2 className="type-heading text-fg">{lab.name}</h2>
                    <p className="mt-1 type-small text-fg-2">{lab.line}</p>
                  </div>
                  <ChevronRightIcon className="shrink-0 text-gold" />
                </Link>
              ) : (
                <div
